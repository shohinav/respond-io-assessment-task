...
const GITHUB_URL = 'https://github.com/shohinav/respond-io-assessment-task'
...
<template>
    <header class="flex h-14 items-center justify-between border-b bg-card px-4">
        <div class="flex items-center gap-3">
            ...
            <span v-if="store.isDirty" class="flex items-center gap-1 text-[10px] font-medium text-amber-600">
        ...
        <div class="flex items-center gap-2">
            <Button variant="ghost" size="icon" :disabled="!store.canUndo" title="Undo (Ctrl+Z)" @click="store.undo()">
                <Undo2 class="size-4" />
            </Button>
            <Button variant="ghost" size="icon" :disabled="!store.canRedo" title="Redo (Ctrl+Shift+Z)"
                @click="store.redo()">
                <Redo2 class="size-4" />
            </Button>
            <AlertDialog v-model:open="resetConfirmOpen">
                ...
                <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction class="bg-destructive text-white hover:bg-destructive/90" @click="reset">
                            Reset
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            ...
            <Button as-child variant="ghost" size="icon">
                <a :href="GITHUB_URL" target="_blank" rel="noreferrer" title="View source on GitHub"
